...
/**
 * To allow a gas to flow in pipes, create a
 * {@link glenn.gasesframework.common.block.BlockGasPipe BlockGasPipe} for the
 * gas.
 * 
 * @author Glenn
 */
public class BlockGas extends Block implements ISample
{
	...
	public AxisAlignedBB getSelectedBoundingBoxFromPool(World world, int i, int j, int k)
	{
		...
		// return AxisAlignedBB.getAABBPool().getAABB((double)i, (double)j +
		// minY, (double)k, (double)i + 1.0D, (double)j + maxY, (double)k +
		// 1.0D);
	...
	/**
	 * Returns the indent to a block at the given coordinate. Should only be
	 * used by the renderer to avoid side clipping.
	 * 
	...
	/**
	 * Called whenever a gas {@link ReactionCommonIgnition reacts} with a block
	 * registered with {@link IGFRegistry#registerIgnitionBlock(Block)}.
	 * 
	 * @param world
	 * @param i
	 * @param j
	...
	/**
	 * Used internally only. Mixes the indices of items of the same value in a
	 * sorted array
	 * 
	 * @param rand
	 * @param valueList
	...
	/**
	 * Used internally only. Fills an array with randomly placed unique indices
	 * with boundaries
	 * 
	 * @param rand
	 * @param indices
	...
	/**
	 * Gets the delay in ticks needed before the block should update again based
	 * on its density. Used internally only.
	 * 
	 * @return
	 */
	...
	public void updateTick(World world, int x, int y, int z, Random random)
	{
		...
		// For technical reasons, metadata is a reverse representation of how
		// much gas there is inside a block
		...
		if (type.density == 0)
		{
			...
			{
				// The gas will flow out from its position, but will prioritize
				// blocks around with the least amount of gas, and especially
				// air. It will not flow into other blocks.
		...
		{
			// If the block in the direction can be flowed into forcefully, it
			// will only move in this direction.
			if (metadata > 0)
			{
		...
		else
		{
			...
			{
				// If the block in the direction is another gas, it will swap
				// the position of the gases according to their densities.
			...
			// If the block in the direction is the same gas, it will attempt to
			// fill the other gas block.
			...
			// If the block in the opposite direction is this gas, it will take
			// the contents of the other block to fill itself
			...
			// The gas will flow out from its position, but will prioritize
			// blocks around with the least amount of gas, and especially air.
			// It will not flow into other blocks.
			...
			// If this block is too small to spread properly, it will attempt to
			// flow along the surface to a gap to be able to move further.
			...
			// Finalizing. Setting the metadata for both this block and the
			// block in the opposite direction to make sure the gases are
			// finite.
			if (metadata > 16)
			{
