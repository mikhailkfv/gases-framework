...
public abstract class TileEntityDirectionalGasPropellor extends TileEntity
{
	...
	/**
	 * Attempt to extract gas from surrounding blocks or whatever the subtype
	 * may do.
	 * 
	 * @return
	 */
	protected GasType extract()
	{
		...
		for (int i = 0; i < 6 & containedType == null; i++)
		{
			...
			if (directionBlock instanceof IGasSource)
			{
				...
				{
					return gasSource.extractGasTypeFromSide(worldObj, x1, y1, z1, direction.getOpposite());
				}
			}
		}
	...
	protected boolean pumpToBlock(int x, int y, int z, ForgeDirection direction)
	{
		...
		return GasesFramework.implementation.tryPushGas(worldObj, worldObj.rand, x, y, z, containedType, direction, pressure);
	}

	/**
	 * Attempt to pump the contained gas type to the specified direction.
